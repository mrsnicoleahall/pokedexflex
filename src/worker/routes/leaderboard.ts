 ...
 * rarity` (default score) and returns the top `LEADERBOARD_LIMIT`, never email
 ...
 * `buildCollectionSummary` is never called here. Trainer Score + Rarity come
 * from ONE `user_ribbons` scan (points derived per row via `pointsForRibbon`,
 * since points are NOT a stored column); Completion + Shiny come from ONE
 * `GROUP BY user_id` aggregate over `specimens` each.
 *
...
leaderboardRoutes.get("/", async (c) => {
  ...
  // 3. Trainer Score + Rarity from ONE user_ribbons scan (points derived per row).
